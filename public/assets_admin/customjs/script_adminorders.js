...
$(document).on('click', '.showUploadsModal_btn', function (e) {
    ...
    let type = 'POST';
    let url = '/admin/showPucUploadPhotosAdmin';
    let message = '';
    let form = '';
    let data = new FormData();
    data.append('puc_id', puc_id);
    // PASSING DATA TO FUNCTION
    SendAjaxRequestToServer(type, url, data, '', showUploadPhotosResponse, '', 'submit_button');

    
});
function showUploadPhotosResponse(response){

    var data = response.data;
    var pucDetail = data.puc_detail;
    var puc_images = pucDetail.puc_images;
    var html = '';
    imageUrlsArray = [];
    var string = pucDetail.registration_number;
    zipFilename = string.replace(/ /g, '-');
     if (puc_images.length > 0) {
        $.each(puc_images, function (index, value) {
            html += `<div class="col-6">
                            <a href="${value.path}" download style="float:right;">
                            ...
                            <img id="puc_vehicle_img" class="clickable-img m-1" src="${value.path}" alt="">
             </div>`;
            
            imageUrlsArray.push(value.path);
        });
        $("#puc_vehicle_img_div").show();
     }else{
        imageUrlsArray = [];
        $("#puc_vehicle_img_div").hide();
     }

     $(".vehicle-photo").html(html);
...
$(document).on('click', '#puc_vehicle_img_down_zip', function (e) {

    if(imageUrlsArray.length > 0){
        downloadVehicleImagesZip(imageUrlsArray,0,zipFilename);
    }
});
$(document).on('click', '#puc_vehicle_img_down_all', function (e) {

    imageUrlsArray.forEach(function(url) {
...
function downloadVehicleImagesZip(imageUrls, count=0, zipFilename='zipFile'){
    ...
    imageUrls.forEach(function(url, index) {
        JSZipUtils.getBinaryContent(url, function (err, data) {
            ...
            zip.file("image" + (index + 1) + ".jpg", data, { binary: true });
            count++;
            if (count === imageUrls.length) {
                // Generate the zip file once all images have been added
