...
class AdminV2Controller extends Controller
{
    ...
    {
        $data['page'] = 'Dashboard';
        return view('admin-v2.dashboard')->with($data);
    }
    ...
    public function getSettingsPageData(Request $request)
    {
        ...
        $data['api_settings'] = ApiSettings::where('type', 'RAZOR_PAY')->first();
    ...
    public function registerUser(Request $request)
    {
        ...
        $Users->fine_rate = $request->puc_fine_rate;
    ...
    public function changePucStatus(Request $request)
    {
        ...
        if ($status_flag == '4') {
            ...
            deleteImage(str_replace(url('/public'),"",$rejected_puc->certificate_pdf));
            ...
            $uploadedImages = PucImages::where('puc_id', $puc_id)->get();
            PucImages::where('puc_id', $puc_id)->delete();
    ...
    public function getAnalyticsPageData(Request $request)
    {
        ...
        $puc_challan_qty = $puc_challan_am = $puc_fine_qty = $puc_fine_am = 0;
        ...
        }else if ($filter_flag == '4') {  // for "All" filter (all pending PUC records)
            // For the "All" filter, we fetch all pending PUC records, regardless of date
            $puc_list = Puc::where('status', 'pending')->get();  // Assuming 'status' is used to filter pending records
    
            // Get the count of all pending PUC records
            $data['pending_puc_all'] = $puc_list->count();
        ...
        if ($puc_list) {
            foreach ($puc_list as $key => $value) {
                ...
                if($value->puc_fine_rate != null){
                    $puc_fine_qty = $puc_fine_qty + 1;
                    $puc_fine_am = $puc_fine_am + $value->puc_fine_rate;
                }
            }
        }
        ...
        $data['puc_fine_qty'] = $puc_fine_qty;
        $data['puc_fine_am'] = $puc_fine_am;

    

        
        $data['total_puc'] = $puc_2w + $puc_3w + $puc_4w;
        return response()->json(['status' => 200, 'message' => "", "data" => $data]);
    }
...
public function showPucUploadPhotosAdmin(Request $request){
