    ...
    var html = '';
     if (puc_images.length > 0) {
        $.each(puc_images, function (index, value) {
            html += `<img id="puc_vehicle_img" class="clickable-img m-1" src="${value.path}" alt="">`;
        });
...
function editSpecificPucResponse(response) {
    ...
    if (response.status == 200 || response.status == '200') {
        ...
        if (puc_detail != null) {
            ...
            $("#puc_fine_rate").val(puc_detail.puc_fine_rate);
            if(puc_detail.puc_fine_rate != null && puc_detail.puc_fine_rate != '0'){
                $("#puc_fine").val('1');
            }else{
                $("#puc_fine").val('0');
            }

            $("#puc_type_opt_"+puc_detail.puc_type_id).addClass('active');
        }
        ...
        return $.Deferred(function(deferred) {
            ...
            reader.onload = function(event) {
                ...
                img.onload = function() {
                    ...
                    }, file.type, 0.8); // Adjust the quality parameter (0.8) as needed
                }
            }
