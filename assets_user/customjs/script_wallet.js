...
$('.add_transaction_btn').click(function (e) {
    var walletAmount = $('.transaction_amount').val();
    if (walletAmount == '') {
    ...
    }else if (walletAmount < minWalletAmount) {
    
        toastr.error('Amount must be minimum '+minWalletAmount+' or above!', '', {
            timeOut: 3000
        });
        return false;
...
$('.addWalletOnlinebtn').on('click', function () {
    ...
    if (walletAmount == '') {
        ...
        $("#online_amount").val(walletAmount);
        setTimeout(function(){
